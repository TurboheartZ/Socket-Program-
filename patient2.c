...
int main(int argc, char *argv[])
{
        ...
        char *pswl;
        int i;
        if((fp=fopen("patient2.txt","r"))==NULL) // Open the file and exit if fail
          {
	...
	buf[numbytes] = '\0';
	printf("Phase 1: Patient2 authentication result: %s\n",buf);
        //**************************Failure Check*******************************************
        int j;
	...
	//****************************RECV the available Msg**********************************
     ...
     while((indexnum<'1')||(indexnum>'6'))
	  ...
	  if((strcmp(avcheck,"notavailable\0"))==0)
	     ...
	     {
			 printf("Phase 2: The requested appointment is available and reserved to Patient2. The assigned doctor port number is %s.\n",avcheck);
		     close(sockfd);
		  }
...
//***********************************Build the UDP with Doc*********************************	
	...
	if (p == NULL) 
	{
		fprintf(stderr, "patient2: failed to bind socket\n");
		return 2;
	}     
...
//*************************************Send the first MSG***********************************
        ...
        if ((fp1=fopen("patient2insurance.txt","r"))==NULL)
           {
...
//**********************************Receive the MSG************************************
	...
	if ((numbytes = recvfrom(usockfd, docname, 6 , 0,
		 (struct sockaddr *)&udptheir_addr, &udpaddr_len)) == -1) 
		 {
		    perror("recvfrom");
		    exit(1);
    ...
    printf("Phase 3: Patient 2 receives %s $ estimation cost from doctor with port number %s and name %s . ", recmsg,docpn,docname);
    printf("\n");
    flag1=0;
