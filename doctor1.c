...
#include <netdb.h>
#include <sys/wait.h>
#define MYPORT "41799"	// the port users will be connecting to
...
//**********************************Gain the insurance info****************************************	
      while(1)
	   ...
	   buf[numbytes] = '\0';
	   printf("Phase 3: Doctor1 receives the request from the patient with port number %d and the insurance plan %s .\n",pport,buf);
	for(m=0;m<=2;m++)
	   {
